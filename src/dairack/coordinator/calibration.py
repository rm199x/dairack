...
class CalibrationEstimate:
    value: float
    role_evidence: float
    kind_evidence: float = 0.0
    kind_weight: float = 0.0

    @property
    def effective_evidence(self) -> float:
        return self.kind_evidence if self.kind_weight else self.role_evidence
...
def estimate(state: dict[str, Any], model: str, role: str, kind: str = "") -> CalibrationEstimate:
    """Estimate a bounded role score with evidence-weighted task-kind backoff."""
    records = state.get("records")
    if not isinstance(records, dict):
        return CalibrationEstimate(0.0, 0.0)
    role_value, role_evidence = _component(records.get(_record_key(model, role)), MAX_ADJUSTMENT)
    if not kind.strip():
        return CalibrationEstimate(role_value, role_evidence)

    kind_value, kind_evidence = _component(
        records.get(_kind_key(model, role, kind)),
        MAX_TOTAL_ADJUSTMENT,
    )
    if kind_evidence < MIN_EVIDENCE:
        return CalibrationEstimate(role_value, role_evidence, kind_evidence)
    kind_weight = kind_evidence / (kind_evidence + PRIOR_EVIDENCE)
    learned = role_value + kind_weight * (kind_value - role_value)
    learned = max(-MAX_TOTAL_ADJUSTMENT, min(MAX_TOTAL_ADJUSTMENT, learned))
    return CalibrationEstimate(learned, role_evidence, kind_evidence, kind_weight)


def adjustment(state: dict[str, Any], model: str, role: str, kind: str = "") -> tuple[float, float]:
    result = estimate(state, model, role, kind)
    return result.value, result.effective_evidence
...
def observe_estimate(
    ...
    *,
    ...
    with _LOCK:
        ...
        return estimate(state, model, role, kind)
...
def observe(
    ...
    *,
    ...
    result = observe_estimate(path, model, role, reward, weight=weight, source=source, kind=kind)
    return result.value, result.effective_evidence
...
def report(path: Path) -> str:
    ...
    rows = [
        ...
        f"Bounded adjustment: +/-{MAX_ADJUSTMENT:.3f} per role, approaching +/-{MAX_TOTAL_ADJUSTMENT:.3f} as task-kind evidence matures",
    ]
    ...
    for record in records.values():
        ...
        evidence = result.effective_evidence
        sources = record.get("sources")
        source_text = (
        ...
        if kind:
            detail = (
                f"  /  role evidence {result.role_evidence:.1f}"
                f"  /  kind evidence {result.kind_evidence:.1f}"
                f"  /  mix {result.kind_weight * 100:.0f}%"
            )
        rendered.append(
            (
                evidence,
                f"{label}  {learned:+.3f}{detail}" + (f"  /  {source_text}" if source_text else ""),
            )
        )
