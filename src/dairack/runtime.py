...
def coordinator_delegation_limit(config: dict[str, Any], route: dict[str, Any] | None = None) -> int:
...
def _specialist_capability_score(capabilities: dict[str, float], specialty: str, task: str) -> float:
...
def coordinator_quality_demand(
    specialty: str,
    task: str,
    context: str = "",
    quality: str = "",
    ...
    semantic = analyze_orchestrator_task([{"role": "user", "content": text}])
    signals = semantic["signals"]
    precision_hits = _signal_hits(
        text,
        (
            "subtle",
            "exact",
...
def select_coordinator_specialist(
    ...
    if policy not in ORCHESTRATOR_POLICIES:
        policy = "adaptive"
    ...
    efficiency_mix = efficiency_mix_by_specialty[specialty][policy]
    if policy == "adaptive":
        efficiency_mix *= 1.0 - quality_demand * 0.74
    elif policy == "efficient":
        efficiency_mix *= 1.0 - quality_demand * 0.28
    resident_bonus = {"efficient": 0.075, "adaptive": 0.035, "quality": 0.008}[policy]
    if policy == "adaptive":
        resident_bonus *= max(0.12, 1.0 - quality_demand * 2.1)
    ranked: list[dict[str, Any]] = []
    preference_role = {"code_review": "coding", "reasoning": "reasoning", "vision": "vision", "general": "general"}[
        specialty
    ]
    preferred_model = _coordinator_role_preference(config, preference_role, models)
    for model in models:
        capabilities = model_capabilities(model)
        metadata = model_capability_metadata(model)
        if specialty == "vision" and not model_supports_vision(provider, model.name):
            continue
        capability_score = _specialist_capability_score(capabilities, specialty, task)
        score = capability_score * (1.0 - efficiency_mix) + capabilities["efficiency"] * efficiency_mix
        ...
        if independent and specialty == "code_review":
            score += 0.055
        elif independent and specialty == "reasoning":
            score += 0.018
    ...
    if not ranked:
        raise RuntimeError(f"no installed model supports the {specialty} delegation")
...
def coordinator_admits_delegation(
...
def execute_coordinator_delegation(
    provider: Any,
    config: dict[str, Any],
    cwd: Path,
    call: dict[str, str],
    ...
    limit = coordinator_delegation_limit(config, parent_route)
    ...
    if raw_path:
        try:
            image_path = resolve_image_path(cwd, raw_path)
    ...
    fingerprint = "|".join((specialty, str(image_path or ""), re.sub(r"\s+", " ", task.lower())[:500]))
    ...
    if image_path is not None and not model_supports_vision(provider, specialist):
        return 1, f"Coordinator selected {specialist}, which cannot accept images.", {}
    admitted, rationale = coordinator_admits_delegation(decision, call, parent_route)
    ...
    runtime = runtime_config_for_model(config, specialist)
    policy = str(decision["policy"])
    if policy == "quality" and specialty == "reasoning":
        supports = getattr(provider, "supports", None)
        if callable(supports):
            try:
    ...
    brief = (
        ...
        f"Delegated specialty: {specialty}\n"
    ...
    try:
        output = _collect_orchestrator_response(
            provider,
            specialist,
            specialist_messages,
            runtime,
    ...
    result = (
        ...
        f"evidence:\n{truncate(strip_tool_markup(output), 12000)}"
    )
...
def coordinator_semantic_assessment(
    ...
    if not isinstance(parsed, dict):
        return {}
    required = set(COORDINATOR_SEMANTIC_SCHEMA["required"])
...
def select_orchestrator_route(
    ...
    if policy not in ORCHESTRATOR_POLICIES:
        policy = "adaptive"
    policy_definition = policy_for(policy)
    tuning = coordinator_tuning_for_config(config)
    try:
        models = list(provider.list_models())
    except Exception as exc:
        route = direct_route(config)
    ...
    try:
        ...
        resident = set()
    analysis = analyze_orchestrator_task(messages, cwd)
    signals = analysis["signals"]
    complexity = float(analysis["complexity"])
    resident_lower = {value.lower() for value in resident}
    learning_state = (
        load_calibration_state(PATHS.state_dir / "coordinator-learning.json")
    ...
    def rank_candidates(
        ...
        ranked: list[dict[str, Any]] = []
        for model in models:
            if float(active_signals.get("vision") or 0) > 0 and not model_supports_vision(provider, model.name):
                continue
            metadata = model_capability_metadata(model)
    ...
    semantic_model = _semantic_router_model(models, resident)
    semantic_context = coordinator_semantic_context(messages)
    semantic_trigger = coordinator_semantic_gate(policy, analysis, score_gap, bool(semantic_context))
    referenced_analysis = (
        coordinator_referenced_task_analysis(messages, cwd)
    ...
    if should_assess and not (cancel_event and cancel_event.is_set()):
        try:
            assessment = coordinator_semantic_assessment(
                provider,
                config,
                semantic_model,
            ...
            assessment = {"error": str(exc), "model": semantic_model}
        if assessment and "error" not in assessment:
            assessment["trigger"] = semantic_trigger
            analysis = _merge_semantic_assessment(analysis, assessment, tuning, referenced_analysis)
            signals = analysis["signals"]
            complexity = float(analysis["complexity"])
    ...
    if use_plan:
        planner = _specialist_model(
            ...
            policy,
            signals,
            _coordinator_role_preference(config, "planner", models),
        )
    if use_review:
        reviewer = _specialist_model(
            provider,
            scored,
            "reviewer",
            executor,
            policy,
            signals,
            _coordinator_role_preference(config, "reviewer", models),
        )
    ...
    return {
        ...
        "signals": signals,
        "execution_scope": coordinator_execution_scope(
            analysis["task_kind"],
            signals,
        ...
        "preference_role": _coordinator_task_role(signals),
        "preferred_model": _coordinator_role_preference(config, _coordinator_task_role(signals), models),
        "candidates": [
            {
                "model": item["model"],
...
def format_route_history(chat: dict[str, Any], limit: int = 12) -> str:
    history = sanitize_route_history(chat.get("route_history"))
    if not history:
...
def coordinator_executor_directive(route: dict[str, Any], config: dict[str, Any]) -> str:
    ...
    limit = coordinator_delegation_limit(config, route)
...
def orchestrator_status(config: dict[str, Any], route: dict[str, Any] | None = None) -> str:
...
def coordinator_learning_path() -> Path:
    return PATHS.state_dir / "coordinator-learning.json"
...
def _collect_orchestrator_response(
    provider: Any,
    model: str,
    messages: list[dict[str, str]],
    config: dict[str, Any],
...
def orchestrator_plan(
    ...
    if not planner:
        return ""
    require_vision_support(provider, planner, messages)
    runtime = runtime_config_for_model(config, planner)
    ...
    return _collect_orchestrator_response(provider, planner, planning_messages, runtime, cancel_event, 360)
...
def orchestrator_review(
    ...
    require_vision_support(provider, reviewer, messages)
    runtime = runtime_config_for_model(config, reviewer)
    task = latest_user_task(messages)
    recent_results = []
    for message in messages[-8:]:
    ...
    raw = _collect_orchestrator_response(provider, reviewer, review_messages, runtime, cancel_event, 320)
    upper = raw.upper()
