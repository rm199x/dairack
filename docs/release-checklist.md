...
- [ ] Test desktop and narrow layouts, modal escape, approval focus, approval-held prompt queues, action cancellation,
  atomic-action labeling, direct command evidence, and patch recovery when those surfaces changed.
...
- [ ] Re-run negative tests for shell composition, network auto-approval, path traversal, no-ripgrep search exclusions,
  redirect handling, bridge authorization, and patch targets.
